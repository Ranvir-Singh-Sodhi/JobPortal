...
def delete_recruiter(request,uid):
    if not request.user.is_authenticated:
        return redirect('admin_login')
    recruiter=User.objects.get(id=uid)
    recruiter.delete()
...
def change_status(request,uid):
    if not request.user.is_authenticated:
        return redirect('admin_login')
    error=""
    recruiter=Recruiter.objects.get(id=uid)
    if request.method=='POST':
...
def add_job(request):
    ...
    if request.method=='POST':
       ...
       recruiter=Recruiter.objects.get(user=user)
       try:
           Job.objects.create(recruiter=recruiter,start_date=sd,end_date=ed,title=jt,salary=sal,image=l,description=des,skills=skills,experience=exp,location=loc,creation_date=date.today())
           error="no"
       except:
           error="yes"
    d={'error': error}
...
def edit_job_details(request,uid):
    if not request.user.is_authenticated:
        return redirect('recruiter_login')
    error=""
...
def change_company_logo(request,uid):
    if not request.user.is_authenticated:
        return redirect('recruiter_login')
    error=""
    job=Job.objects.get(id=uid)
...
def job_details(request,uid):
    job=Job.objects.get(id=uid)
    d={'job':job}
    return render(request,'job_details.html',d)

def apply_for_job(request,uid):
    if not request.user.is_authenticated:
        return redirect('user_login')
    error=""
    user=request.user
